...
interface DatasetEducationRepo : CrudRepository<DDatasetRecord, Int> {
    ...
    @Query("SELECT SUM(e.value) / SUM(p.value) as count, g.lat, g.lon FROM education e NATURAL JOIN geocode_lut g, population p WHERE e.meta IN :metas AND e.age IN :ages AND e.sex = :sex AND e.geocode = g.geocode AND p.geocode = g.geocode GROUP BY g.geocode, g.lon, g.lat", nativeQuery = true)
